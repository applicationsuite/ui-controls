...
export interface IDataGridColumn<T = unknown> {
	...
	hideable?: boolean;
	/**
	 * Required columns can never be hidden. They appear in the column-visibility
	 * picker as a checked-and-disabled entry, and are forced visible at runtime
	 * even if `initialState.hiddenColumns` includes them. Implies `defaultVisible`.
	 */
	required?: boolean;
	/**
	 * Initial visibility when the grid first mounts. Default: true. Required
	 * columns ignore this and are always visible.
	 */
